"""Extraction history tracking (``audio_extract_history.csv``) for the audio stage."""
...
class ExtractHistory:
    """CSV-backed record of extracted videos, used to skip already-extracted ones."""
    ...
    def _load_existing(self) -> None:
        ...
        logger.debug("Loaded %d previously extracted video(s) from history", len(self._completed_ids))

    def is_extracted(self, video_id: str) -> bool:
        return video_id in self._completed_ids

    def record(self, entry: ExtractRecord) -> None:
