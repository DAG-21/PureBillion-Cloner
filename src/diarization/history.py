"""Diarization history tracking (``diarization_history.csv``) for the diarization stage."""
...
HISTORY_FIELDS = ["video_id", "source_audio", "status", "output_path", "error", "timestamp"]
...
class DiarizationHistory:
    """CSV-backed record of diarized audio files, used to skip already-diarized ones."""
    ...
    def _load_existing(self) -> None:
        ...
        logger.debug("Loaded %d previously diarized audio file(s) from history", len(self._completed_ids))

    def is_diarized(self, video_id: str) -> bool:
        return video_id in self._completed_ids

    def record(self, entry: DiarizationRecord) -> None:
